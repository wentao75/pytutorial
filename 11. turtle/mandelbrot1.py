...
def mandelbrot_edge(resolution, iterations, edge):
    points = []
    x_coordinates = [
        -2 + i * (3 / resolution) for i in range(int(resolution * 1.5))
    ]
...
def draw_boundary(protagonist, boundary_points, speed):
    ...
    protagonist.penup()
    protagonist.setheading(
        protagonist.towards(boundary_points[0][0], boundary_points[0][1]))
    protagonist.forward(
        protagonist.distance(boundary_points[0][0], boundary_points[0][1]))
    protagonist.pendown()
...
def main():
    setting = Screen()
    setting.title(
        "Julia T. Gaston and the Case of the Complex Quadratic Polynomial")
    setting.bgcolor('#C2EBFF')
    protagonist = RawTurtle(setting)
    protagonist.shape("turtle")
    points = [(190 * p[0], 190 * p[1])
              for p in mandelbrot_edge(300, 500, list(range(60, 500)))]
